...
module.exports = (bot, logger, utils) => {
	bot.on('message', async msg => {
		...
		const getLanguage = await utils.getLanguage(msg.from.language_code, msg.from.id, 'get');
		...
		if(msgText.startsWith('get ')) {
			...
			if(!id) return bot.sendMessage(msg.chat.id, getLanguage('wrongNumber'), { reply_to_message_id: msg.message_id });
			return bot.sendMessage(msg.chat.id, getLanguage('result', readableId), { reply_to_message_id: id }).catch(() => {
				return bot.sendMessage(msg.chat.id, getLanguage('noResult', readableId), { reply_to_message_id: msg.message_id });
			})
		}
	});
}