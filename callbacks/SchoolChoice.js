module.exports = (telegram, logger, utils) => {
	telegram.events.on('callback_query', async msg => {
		if(msg.data.action === 'SchoolChoice') {
			logger.log('notice', 'User %s Used School Choice Button(School %s, Type %s) in %s(%s)', `${msg.from.parsed_username}(${msg.message.from.id})`, msg.data.code, msg.data.type, msg.message.chat.title, msg.message.chat.id);
			const meal = await new (require('../modules/SchoolMeal'))().getMeal(msg.data.type, msg.data.code).catch(e => {
				switch(e) {
