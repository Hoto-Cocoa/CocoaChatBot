module.exports = class Language {
	...
	async getLanguage(langCode, userId, msgCategory) {
		var languageSetting = await this.database.query('SELECT value FROM setting WHERE userId=? AND `key`="language" AND active=1;', userId), languageData = null;
		langCode = languageSetting ? languageSetting : langCode ? langCode.substring(0, 2) : 'en';
		languageData = this.languages[langCode] ? this.languages[langCode] : this.languages['en'];
		return (msgCode, ...args) => {
